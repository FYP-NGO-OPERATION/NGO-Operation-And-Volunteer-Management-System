...
- **Global scope**: `E:/Fyp/ngo_volunteer_app/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `E:/Fyp/ngo_volunteer_app/.claude/notes/`)
...
2. Glob `E:/Fyp/ngo_volunteer_app/.claude/notes/*.md` (if directory exists) — global notes
...
Global (E:/Fyp/ngo_volunteer_app/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `E:/Fyp/ngo_volunteer_app/.claude/notes/` — works in any directory
