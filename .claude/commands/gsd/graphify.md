...
**CJS-only (graphify):** `graphify` subcommands are not registered on `gsd-sdk query`. Use `node E:/Fyp/ngo_volunteer_app/.claude/get-shit-done/bin/gsd-tools.cjs graphify …` as documented in this command and in `docs/CLI-TOOLS.md`. Other tooling may still use `gsd-sdk query` where a handler exists.
...
Knowledge graph is disabled. To activate:
  ...
  node E:/Fyp/ngo_volunteer_app/.claude/get-shit-done/bin/gsd-tools.cjs config-set graphify.enabled true
...
```bash
node E:/Fyp/ngo_volunteer_app/.claude/get-shit-done/bin/gsd-tools.cjs graphify query <term>
```
...
```bash
node E:/Fyp/ngo_volunteer_app/.claude/get-shit-done/bin/gsd-tools.cjs graphify diff
```
...
```
PREFLIGHT=$(node "E:/Fyp/ngo_volunteer_app/.claude/get-shit-done/bin/gsd-tools.cjs" graphify build)
```
...
Project root: ${CWD}
gsd-tools path: E:/Fyp/ngo_volunteer_app/.claude/get-shit-done/bin/gsd-tools.cjs
...
4. **Write diff snapshot:**
   ```
   node \"E:/Fyp/ngo_volunteer_app/.claude/get-shit-done/bin/gsd-tools.cjs\" graphify build snapshot
   ```
