...
class _AdminLayoutState extends State<AdminLayout> {
        ...
        .listen((snapshot) {
          for (var change in snapshot.docChanges) {
            if (change.type == DocumentChangeType.added) {
              ...
              _showSosEmergencyDialog(
                data['userName'] ?? 'A Volunteer',
                change.doc.id,
              );
            }
          }
  ...
  void _showSosEmergencyDialog(String userName, String docId) {
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text(
          '$userName has triggered an SOS Alert! They need immediate assistance.',
  ...
  Widget _buildAdminProfile() {
    return ProfileTab(onLogout: _handleLogout);
  }
        ...
        if (mounted) {
          showDialog(
            ...
            builder: (context) => AlertDialog(
              ...
              content: const Text(
                'Please allow microphone access in your device settings to use voice commands.',
              ),
              actions: [
                TextButton(
                  onPressed: () async {
  ...
  void _processVoiceCommand(String text) {
    ...
    if (text.contains('route') || text.contains('tsp')) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Voice Command: Opening Route Optimization'),
        ),
      );
    } else if (text.contains('sentiment') || text.contains('ai')) {
      Navigator.push(
        context,
        MaterialPageRoute(builder: (_) => const SentimentAnalysisScreen()),
      );
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Voice Command: Opening AI Sentiment')),
      );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Voice Command: Opening Disaster Map')),
      );
    }
  }
      ...
      child: Scaffold(
        appBar: isDesktop
            ...
            : AppBar(
                ...
                backgroundColor:
                    Provider.of<DisasterProvider>(context).isEmergencyMode
                    ? Colors.red
                    : null,
                actions: [
                  IconButton(
                    icon: Icon(
                      ...
                      color: _isListening ? Colors.amber : null,
                    ),
                  ...
                  Consumer<DisasterProvider>(
                    builder: (context, dp, _) => IconButton(
                      icon: Icon(
                        ...
                        color: dp.isEmergencyMode ? Colors.yellow : null,
                      ),
                        ...
                        if (!dp.isEmergencyMode) {
                          final confirm = await showDialog<bool>(
                            ...
                            builder: (context) => AlertDialog(
                              ...
                              content: const Text(
                                'This will alert all volunteers and change the UI to disaster mode. Are you sure?',
                              ),
                              actions: [
                                TextButton(
                                  onPressed: () =>
                ...
                onDestinationSelected: (index) {
                  if (index == 8) {
                    ...
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (_) => const BloodEmergencyScreen(),
                      ),
                    );
                  } else if (index == 12) {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (_) => const SentimentAnalysisScreen(),
                      ),
                    );
                  } else {
                    setState(() => _selectedIndex = index);
                ...
                children: [
                  ...
                  ...List.generate(12, (index) {
                    final starAngle =
                        ...
                        (index * math.pi / 4);
                    final opacity =
                        (math.sin(starAngle * (2 + index % 3)) + 1) / 2 * 0.9;
